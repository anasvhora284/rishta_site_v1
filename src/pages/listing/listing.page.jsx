...
const Listing = (
  ...
  return (
    <div className="MainListingPage">
      ...
      {filteredData?.length ? <>{filteredData?.map((obj, index) => {
        return (
          <div
        ...
        <Box className="no-data-inner-box">
          ...
          <p className="no-result-sub-text">Try adjusting your filter <br />to find what you are looking for</p>
          <button
            onClick={() => {
              navigate("/filter")
            }}
            className="filter-again-btn">Filter again</button>
        </Box>
      </Box></>}
    </div>
  );
