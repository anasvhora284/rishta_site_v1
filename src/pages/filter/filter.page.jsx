      ...
      try {
        ...
        if (excelData && excelData.length > 0) {
          ...
          setQualificationValues((prevQualifications) => {
            ...
            const specialElements = uniqueQualifications.filter(
              (qualification) => specialCases.includes(qualification)
            );

            // Filter out the special cases from the main array
            const remainingElements = uniqueQualifications.filter(
              (qualification) => !specialCases.includes(qualification)
            );
            ...
            console.log(sortedQualifications, "sortedQualifications");
    ...
    const outputData = excelData.filter((data) => {
      ...
      return (
        (!qualification?.length ||
          qualification.includes(data?.["Education Category"])) &&
